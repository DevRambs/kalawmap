...
import { X, Menu, User, MapPin, Calendar, AlertTriangle, Eye, Trash2, LogIn, LogOut, Shield, FileText, Settings, BarChart3, Users, Home, Search, Filter, Edit, Info } from 'lucide-react';
...
export const Sidebar: React.FC<SidebarProps> = ({
  ...
  const [floodIncidentsCount, setFloodIncidentsCount] = useState(0);
  ...
  // Fetch flood incidents count
  useEffect(() => {
    const fetchFloodIncidents = async () => {
      try {
        ...
        setFloodIncidentsCount(querySnapshot.size);
      } catch (error) {
  ...
  const menuItems = [
    ...
    // Only show Flood Records for authenticated users
    ...(user ? [{
      id: 'incidents',
      label: 'Flood Records',
      icon: FileText,
      badge: floodIncidentsCount, // Show count of flood incidents instead of monitored areas
      action: () => {
        onShowIncidents();
      }
    }] : []),
    ...
    // Only show System Settings for authenticated users
    ...(user ? [{
      id: 'settings',
    ...
    <>
      ...
      <div className="fixed top-4 left-4 z-50">
        <button
          ...
          className="p-3 bg-white/10 hover:bg-white/20 backdrop-blur-md rounded-xl shadow-lg border border-white/30 transition-all duration-200 hover:scale-105 hover:shadow-xl"
