...
function App() {
  ...
  // Marker size control state variables
  const [baseMarkerSize, setBaseMarkerSize] = useState(18);
  const [glowMarkerSize, setGlowMarkerSize] = useState(30);
  ...
  // Show auth page if requested or if authentication is required for protected pages
  if (showAuthPage || (!user && (showAdminPage || showIncidentsPage || showAnalyticsPage || showSettingsPage))) {
    return <AuthPage onBack={() => {
      setShowAuthPage(false);
      setShowAdminPage(false);
      setShowIncidentsPage(false);
  ...
  if (showIncidentsPage) {
    return <FloodIncidentsPage onBack={() => {
      setShowIncidentsPage(false);
      setSidebarOpen(true);
    }} />;
  }
    ...
    <div className="relative w-full h-screen overflow-hidden">
      <Map
        ...
        style={{ 
          width: '100%', 
          height: '100%',
          cursor: drawingState.currentTool === 'polygon' ? 'url(data:image/svg+xml;utf8,<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="%23ffffff" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M12 19l7-7 3 3-7 7-3-3z"/><path d="M18 13l-1.5-7.5L2 2l3.5 14.5L13 18l5-5z"/><path d="M2 2l7.586 7.586"/><circle cx="11" cy="11" r="2"/></svg>), auto' 
            : drawingState.currentTool === 'marker' ? 'url(data:image/svg+xml;utf8,<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="%23ffffff" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M21 10c0 7-9 13-9 13s-9-6-9-13a9 9 0 0 1 18 0z"></path><circle cx="12" cy="10" r="3"></circle></svg>), auto'
            : drawingState.currentTool === 'delete' ? 'url(data:image/svg+xml;utf8,<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="%23ffffff" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polyline points="3 6 5 6 21 6"></polyline><path d="M19 6v14a2 2 0 0 1-2 2H7a2 2 0 0 1-2-2V6m3 0V4a2 2 0 0 1 2-2h4a2 2 0 0 1 2 2v2"></path><line x1="10" y1="11" x2="10" y2="17"></line><line x1="14" y1="11" x2="14" y2="17"></line></svg>), auto'
            : 'default'
        }}
        ...
        <Source id="flood-risk-areas" type="geojson" data={floodRiskAreasGeoJSON}>
          ...
          <Layer
            ...
            paint={{
              ...
              'circle-radius': [
                'case',
                ['get', 'isSevereRisk'], baseMarkerSize + 4,
                ['get', 'isHighRisk'], baseMarkerSize + 3,
                ['get', 'isModerateRisk'], baseMarkerSize + 2,
                baseMarkerSize
              ],
          ...
          <Layer
            ...
            paint={{
              ...
              'circle-radius': [
                'case',
                ['get', 'isSevereRisk'], 
                ['+', glowMarkerSize, ['*', 8, ['+', 1, ['sin', ['*', ['get', 'animationTime'], 4]]]]],
                ['+', glowMarkerSize - 2, ['*', 6, ['+', 1, ['sin', ['*', ['get', 'animationTime'], 3]]]]]
              ],
              'circle-opacity': [
                'case',
                ['get', 'isSevereRisk'], 
          ...
          <div className="flex flex-col gap-3">
            ...
            <div className="flex flex-col">
              <button
                ...
                className={`p-3 ${buttonClasses} rounded-t-lg border-b ${isDarkTheme ? 'border-gray-600/50' : 'border-white/20'} 
              ...
              <button
                ...
                className={`p-3 ${buttonClasses} rounded-b-lg 
                         ${textClasses} transition-all duration-200 hover:scale-105 active:scale-95`}
                title="Zoom Out"
              >
            ...
            <div className="relative">
              <button
                ...
                className={`p-3 ${buttonClasses} hover:bg-green-500/50 rounded-lg
                         ${textClasses} transition-all duration-200 hover:scale-105 active:scale-95
              ...
              {showBasemapMenu && (
                <div className={`absolute right-0 top-full mt-2 ${panelClasses} min-w-48 z-20`}>
                  {Object.entries(BASEMAPS).map(([key, basemap]) => (
                    <button
                      key={key}
                      onClick={() => handleBasemapSelect(key as BasemapKey)}
                      className={`w-full text-left px-3 py-3 text-sm ${textClasses} ${isDarkTheme ? 'hover:bg-gray-700/80' : 'hover:bg-white/20'} 
